...
(function (global, $) {
  ...
  if (env === 'dev') {
	...
	var capactiy_info = function (options) {
		...
		var headtpl = _render.renderHeadTpl(version_name, edition_id, file_status, av_status, time_status, is_test, isCenter, remain_day, capacity_packet, idcode);
		...
		var EditionModaltpl = _render.renderEditionModal(isCenter, edition_id, remain_day, end_time, time_status, version_name, edition_old);
	...
	var _render = {
		...
		renderHeadTpl: function (version_name, edition_id, file_status, av_status, time_status, is_test, isCenter, remain_day, capacity_packet, idcode) { // 渲染头部
			var headertpl = _render.HandleHeaderTpl(version_name, edition_id, file_status, av_status, time_status, is_test, isCenter, remain_day, capacity_packet, idcode);
			return headertpl;
		},
		HandleHeaderTpl: function (version_name, edition_id, file_status, av_status, time_status, is_test, isCenter, remain_day, capacity_packet, idcode) { // 头部条渲染模版
			...
			/* // 续费按钮
			var version_up = (edition_id === '4' && is_test && is_test === '1') ? '升级' : '续费';

			if (edition_id !== '1') {
				if ((edition_id === '3' || edition_id === '2') && (file_status >= 2 || av_status >= 2)) {
					// ((edition_id === '3' || edition_id === '2') && (file_status < 2 && av_status < 2) && time_status === 1) ? (tpl += '<span class="xufeibefore"></span>') : ''; // 续费前的线
					// tpl += '<a buy_click_record iscenter='+ isCenter +' target="_blank" href="'+ USER_DOMAIN +'/buy?create_from='+(isCenter == 0 ? '3' : '2')+'" class="text-blue up-edititon '+ (time_status === 2 || ((edition_id === '4' && is_test && is_test === '1')) ? 'active' : '') +'">'+ version_up +'</a>';
				} else {
					if (edition_id === '15' && capacity_packet.idcode > 0) {
						tpl += '<a buy_click_record iscenter=' + isCenter + ' target="_blank" href="' + USER_DOMAIN + '/buy?create_from=' + (isCenter == 0 ? '3' : '2') + '&version=15&auth_members=' + (Number(capacity_packet.idcode) / 5)+ '" class="text-blue upgrade-edititon ' + (time_status === 2 || ((edition_id === '4' && is_test && is_test === '1')) ? 'active' : '') + '">' + version_up + '</a>';
					} else {
						tpl += '<a buy_click_record iscenter=' + isCenter + ' target="_blank" href="' + USER_DOMAIN + '/buy?create_from=' + (isCenter == 0 ? '3' : '2') + '&version=' + edition_id + '" class="text-blue upgrade-edititon">' + version_up + '</a>';
					}
				}
			} else {
				// TODO: 改链接
				((edition_id === '3' || edition_id === '2') && (file_status !== 2 && av_status !== 2)) ? (tpl += '<span class="xufeibefore"></span>') : ''; // 续费前的线
				tpl += '<a buy_click_record iscenter=' + isCenter + ' target="_blank" href="' + CLI_DOMAIN + '/price" class="text-blue upgrade-edititon">升级</a>';
			}

			// 升级按钮
			if (edition_id !== '1' && edition_id !== '4' && edition_id !== '44' && edition_id !== '15') {
				if ((time_status == 1) || time_status == 2 && (file_status >= 2 || av_status >= 2)) {
					((edition_id === '3' || edition_id === '2') && (file_status !== 2 && av_status !== 2) && time_status === 1) ? (tpl += '<span class="xufeibefore"></span>') : ''; // 续费前的线
					tpl += '<a href="' + USER_DOMAIN + '/buy?create_from=' + (isCenter == 0 ? '3' : '2') + '&version=' + (edition_id == '2' ? '3' : '15') + '" target="_blank" class="text-blue upgrade-edititon ' + ((file_status >= 2 || av_status >= 2) ? 'active' : '') + '">升级</a>';
				}
			} */
		...
		renderUpVersion: function (isCenter) {
			var tpl = '<div id="modal-up-version" class="modal fade modal-up-version capacity-nav-info-modal" data-backdrop="true">' +
				...
				'<div class="modal-explain">注：补差价升级后的版本到期时间不变，如需延长当前版本的到期时间，可点击<a buy_click_record iscenter=' + isCenter + ' href="' + USER_DOMAIN + '/buy?create_from=' + (isCenter == 0 ? '3' : '2') + '" class="text-blue" target="_blank"> 续费</a></div>' +
		...
		renderEditionModal: function (isCenter, edition_id, remain_day, end_time, time_status, version_name, edition_old) {
			...
			if (time_status == 3) {
				EditionModaltpl += '<ul class="modal-expired_advantage">' +
					...
					'<li>音视频流量若有剩余，所有音视频可继续播放，具体<a href="https://cli.im/help/48183" target="_blank" statisticAnalyze sec-data="55" class="link text-blue">了解详情</a>;</li>' +
					'<li>图片/文件容量，恢复为 50M；</li>' +
			...
			EditionModaltpl += '</div>';
			var goVersion = edition_old ? (edition_old.edition_id > 1 ? edition_old.edition_id : '2') : '2';
			...
			EditionModaltpl += '<div class="modal-expired_buttons">' +
				...
				'<a statisticAnalyze sec-data="21" href="' + USER_DOMAIN + '/buy?create_from=' + (isCenter == 0 ? '3' : '2') + '&version=' + goVersion + '" iscenter=' + isCenter + ' target="_blank" class="btn green b-green btn-sm">立即续费</a>' +
				'</div>' +
				'<p class="modal-expired_phone text-darkgrey text-center">专属电话：15867579147</p>' +
				'</div>' +
				'</div>' +
				'</div>';
