$(function() {
    ...
    if (typeof _capacity_info != 'undefined') {
        if (_capacity_info && _capacity_info != null) {
            ...
            var sec = '';
            if (editionType === 3 || editionType === 15) {
                popContent = '容量即将超标/已经超标：补差价升级获取更多容量';
            } else if (editionType === 4 || editionType === 16) {
                popContent = '流量即将超标/已经超标：补差价升级获取更多流量';
            } else if (editionType === 8 || editionType === 12) {
                popContent = '流量即将超标/已经超标：购买扩容包获取更多流量';
            } else if (editionType === 7 || editionType === 11) {
                popContent = '容量即将超标/已经超标：购买扩容包获取更多容量';
            } else if (editionType === 6 || editionType === 10) {
                popContent = '购买扩容包获取更多容量/流量/成员数量';
            }
            if (editionType === 1 || editionType === 14 || editionType === 15 || editionType === 16) {
            ...
            if (!checkLogin()) {
                if ($login_join.data("hasClickJoin") == "lock") { //请求开始加锁
                    ...
                    if ($('.login_plugins_bg').length == 0) { // 点击注册/登录，弹出多个弹窗bug
                        ...
                        seajs.use('//' + STATIC_SERVICE + '/js/login_plugin.js?v=20190604', function(cli) {
                            if (data_type == 'login') {
